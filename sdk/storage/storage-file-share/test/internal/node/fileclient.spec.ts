...
describe.runIf(getAccountKey())("FileClient", () => {
  ...
  it("create directory - name with directory dots", async () => {
    const dirBaseName = getUniqueName("dirname1", { recorder });
  ...
  it("setHTTPHeaders with all parameters set", async () => {
    ...
    const headers = {
      ...
      fileContentEncoding: "fileContentEncoding",
      fileContentLanguage: "fileContentLanguage",
      fileContentMD5: Buffer.from([1, 2, 3, 4]),
      fileContentType: "fileContentType",
    };
  ...
  it("startCopy - with sddl file permission", async () => {
    ...
    const newFileClient = dirClient.getFileClient(getUniqueName("copiedfile", { recorder }));
  ...
  it.skipIf(isLiveMode())("getRangeListDiff", async function () {
  ...
  it.skipIf(isLiveMode())("getRangeListDiff with share snapshot", async function () {
    await fileClient.create(512 * 4 + 1);
    await fileClient.uploadRange("Hello", 0, 5);
  ...
  it.skipIf(isLiveMode())("getRangeListDiff with rename", async function () {
    await fileClient.create(512 * 4 + 1);
    await fileClient.uploadRange("Hello", 0, 5);
    ...
    const newFileName = getUniqueName("rename_file", { recorder });
  ...
  it.skipIf(isLiveMode())("getRangeListDiff with share snapshot and rename", async function () {
    await fileClient.create(512 * 4 + 1);
    await fileClient.uploadRange("Hello", 0, 5);
  ...
  it.runIf(isLiveMode())("download should update progress and abort successfully", async () => {
  ...
  it("listHandles for file with Invalid Char should work", async function () {
    const fileNameWithInvalidChar = getUniqueName("file\uFFFE", { recorder });
  ...
  it("rename - source leased", async () => {
    ...
    const leaseClient = sourceFileClient.getShareLeaseClient(guid);
    const leaseResult = await leaseClient.acquireLease(-1);
  ...
  it("rename - Non-ASCII source and destination", async () => {
    ...
    const destFileName = getUniqueName(destName, { recorder });

    const sourceName = "汉字. source ~!@#$%^&()_+`1234567890-={}[];','";
    const sourceFileName = getUniqueName(sourceName, { recorder });
    const sourceFileClient = shareClient.getDirectoryClient("").getFileClient(sourceFileName);
...
describe.runIf(getAccountKey())("FileClient - OAuth", () => {
  let shareName: string;
  let shareClient: ShareClient;
  ...
  beforeEach(async (ctx) => {
    ...
    const shareServiceClient = await createShareServiceClient("SharedKeyCredential", { recorder });
    assert.isDefined(shareServiceClient);
    shareClientWithKeyCredential = shareServiceClient.getShareClient(shareName);
...
describe.runIf(getAccountKey())("FileClient - AllowTrailingDots - True", () => {
  let shareName: string;
  let shareClient: ShareClient;
  let dirName: string;
  let dirClient: ShareDirectoryClient;
  ...
  it("startCopyFromURL", async () => {
    await fileClient.create(1024);
    const copiedFileName = getUniqueName("copiedfile", { recorder });
  ...
  it("rename", async () => {
    ...
    const anotherDestFileName = getUniqueName("destfile1", { recorder }) + "....";
...
describe.runIf(getAccountKey())("FileClient - AllowTrailingDots - False", () => {
  let shareName: string;
  let shareClient: ShareClient;
  let dirName: string;
  let dirClient: ShareDirectoryClient;
  ...
  it("rename", async () => {
    await fileClient.create(1024);
    const destFileBaseName = getUniqueName("destfile", { recorder });
    ...
    const firstDestClient = result.destinationFileClient;
    const anotherDestFileBaseName = getUniqueName("destfile1", { recorder });
...
describe.runIf(getAccountKey())("FileClient - AllowTrailingDots - Default", () => {
  let shareName: string;
  let shareClient: ShareClient;
  let dirName: string;
  let dirClient: ShareDirectoryClient;
...
describe("FileClient - NFS", () => {
  ...
  it("createHardLink", async () => {
    ...
    const hardLink = getUniqueName("hardLink", { recorder });
