...
describe("LeaseClient", () => {
  ...
  it("acquireLease again with another lease id", async () => {
    ...
    const anotherLeaseClient = fileClient.getShareLeaseClient(anotherGuid);
    try {
  ...
  it("changeLease before acquiring a lease", async () => {
    const leaseClient = fileClient.getShareLeaseClient(guid);
    try {
      const changeResp = await leaseClient.changeLease(anotherGuid);
      assert.equal(changeResp.leaseId, anotherGuid);
      await leaseClient.releaseLease();
    } catch (err: any) {
      assert.equal(err.statusCode, 409);
    }
  ...
  it("getRangeList", async () => {
    ...
    try {
      await fileClient.getRangeList({ leaseAccessConditions: { leaseId: anotherGuid } });
      assert.fail(
  ...
  it("download file", async () => {
    ...
    try {
      await fileClient.download(0, undefined, { leaseAccessConditions: { leaseId: anotherGuid } });
      assert.fail(
        "The lease ID specified in the request should matches that of the file if specified",
      );
    } catch (err: any) {
  ...
  it("getProperties", async () => {
    ...
    try {
      await fileClient.getProperties({ leaseAccessConditions: { leaseId: anotherGuid } });
      assert.fail(
        "The lease ID specified in the request should matches that of the file if specified",
      );
    } catch (err: any) {
...
describe("LeaseClient with ShareClient", () => {
  ...
  it("acquireLease again with another lease id", async () => {
    ...
    const anotherLeaseClient = shareClient.getShareLeaseClient(anotherGuid);
    try {
      await anotherLeaseClient.acquireLease(duration);
      assert.fail("acquireLease a leased lease should fail with a different lease id");
    } catch (err: any) {
  ...
  it("changeLease before acquiring a lease", async () => {
    ...
    try {
      await leaseClient.changeLease(anotherGuid);
    } catch (err: any) {
      assert.equal(err.statusCode, 409);
    }
  });
  ...
  it("getProperties", async () => {
    ...
    try {
      await shareClient.getProperties({ leaseAccessConditions: { leaseId: anotherGuid } });
      assert.fail(
        "The lease ID specified in the request should matches that of the file if specified",
      );
    } catch (err: any) {
  ...
  it("getStatistics", async () => {
    ...
    try {
      await shareClient.getStatistics({ leaseAccessConditions: { leaseId: anotherGuid } });
      assert.fail("lease id required if the file has an active lease");
    } catch (err: any) {
      assert.equal(err.statusCode, 412);
    }
