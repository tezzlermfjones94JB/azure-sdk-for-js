...
describe("BlobServiceClient", () => {
  ...
  it("Customized audience should work", async () => {
    const serviceClient = await createBlobServiceClient("TokenCredential", {
      recorder,
      options: {
        audience: [getBlobServiceAccountAudience(getAccountName())],
      },
    });
    await serviceClient.getProperties();
  });
  ...
  it("can be created with a url and a pipeline", async () => {
    const newClient = await createBlobServiceClient("Pipeline", { recorder });
  ...
  it.skip("ListContainers including system containers", async function () {
    const blobServiceClient = await createBlobServiceClient("TokenCredential", { recorder });
  ...
  it("ListContainers with all parameters configured", async () => {
    ...
    const containerNamePrefix = getUniqueName("container", { recorder });
  ...
  it("getUserDelegationKey should work", async function () {
    ...
    const response = await blobServiceClient.getUserDelegationKey(now, tmr);
  ...
  it("Find blob by tags should work", async () => {
    ...
    const key1 = getUniqueName("key1", { recorder });
  ...
  it("restore container", async function () {
    ...
    for await (const c of blobServiceClient.listContainers({
      ...
      if (c.deleted && c.name === containerName) {
        ...
        assert.equal(undeleted.containerName, containerName);
        assert.isTrue(
          await undeleted.exists(),
          "Restored container never became available before timeout",
        );
        break;
      }
    }
