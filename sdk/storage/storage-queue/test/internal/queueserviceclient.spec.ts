...
describe("QueueServiceClient", () => {
  ...
  it("listQueues with all parameters", async () => {
    ...
    const queueNamePrefix = getUniqueName("queue", { recorder });
  ...
  it("getStatistics with default/all parameters secondary", async () => {
    const alternateUrl = getSecondaryAccountQueueUrl();

    const queueServiceClient = new QueueServiceClient(alternateUrl, createTestCredential());
    await ensureClientRecording(recorder, queueServiceClient);
  ...
  it("can create and delete a queue", async () => {
    ...
    const result = await queueServiceClient.getQueueClient(queueName).getProperties();
...
describe("QueueServiceClient - Verify Name Properties", () => {
